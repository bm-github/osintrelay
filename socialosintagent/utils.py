...
def sanitize_username(username: str) -> str:
    ...
    sanitized_user = "".join(
        ch for ch in normalized_user if unicodedata.category(ch)[0] != "C"
    )
    if sanitized_user != username:
        logger.info(
            f"Sanitized username. Original: '{username}', Sanitized: '{sanitized_user}'"
        )
...
def handle_rate_limit(
    platform_context: str, exception: Exception, should_raise: bool = True
):
    """Log rate-limit details. Headless-safe (no Rich / console)."""
        ...
        if hasattr(original_exc, "response") and original_exc.response:
            ...
            if reset_timestamp and reset_timestamp.isdigit():
                ...
                if wait_duration.total_seconds() > 0:
                    ...
                    reset_info = (
                        f"The API rate limit will reset in approximately {minutes} minute(s) "
    ...
    if should_raise:
        raise RateLimitExceededError(
            error_message + f" ({reset_info})", original_exception=original_exc
        )
