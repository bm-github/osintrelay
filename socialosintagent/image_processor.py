...
class ImageProcessor:
    ...
    async def preprocess_image_async(
        ...
        """
        Async version of preprocess_image for parallel processing.

        Runs PIL operations in a thread pool to avoid blocking the event loop.
        ...
        Returns:
            Path to processed image, or None on failure
        """
        return await asyncio.to_thread(self.preprocess_image, file_path, output_path)
    ...
    async def process_single_image_async(
        ...
        """
        Async version of process_single_image for parallel processing.
        ...
        # Preprocess async
        processed_path = await self.preprocess_image_async(file_path)
        if not processed_path:
            return ImageProcessingResult(
                url=url,
                status=ProcessingStatus.PREPROCESSING_FAILED,
        ...
        # Perform analysis (this is still sync as it's an LLM API call)
        # We keep it sync to respect rate limits
        try:
            analysis = analyze_func(processed_path, source_url=url, context=context)
