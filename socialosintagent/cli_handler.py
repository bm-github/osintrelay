...
class CliHandler:
    ...
    def run(self):
        ...
        if self.args.offline:
            self.console.print(
                Panel(
                    "[bold yellow]OFFLINE MODE ENABLED[/bold yellow]\nData will be sourced only from local cache. No new data will be fetched.",
                    title_align="center",
                    border_style="yellow",
                )
            )
    ...
    def _show_main_menu(self):
        ...
        if not available:
            self.console.print(
                "[red]No platforms are configured correctly. Please check your .env file.[/red]"
            )
            return
        ...
        if len(available) > 1:
            ...
            n += 1
        opts.update(
            {str(n): "purge data", str(n + 1): "cache status", str(n + 2): "exit"}
        )
        ...
        selected_platforms = (
            available
            if opts.get(choice) == "cross-platform"
            else [
                opts[k]
                for k in choice.split(",")
                if k.strip() in opts and opts[k.strip()] in available
            ]
        )
    ...
    def _collect_targets_and_start_session(self, selected_platforms: List[str]):
        ...
        """
        self.console.print(
            f"Selected: {', '.join(p.capitalize() for p in selected_platforms)}"
        )
        ...
        for p in selected_platforms:
            ...
            users = [
                sanitize_username(u.strip())
                for u in users_input.split(",")
                if u.strip()
            ]
            if users:
                self.console.print(
                    Text("Cache check: ", style="dim")
                    + Text.from_markup(
                        ", ".join(
                            [f"{u} {self._get_cache_info_string(p, u)}" for u in users]
                        )
                    )
                )
        ...
        MIN_FETCH_ALLOWED = 5
        default_count_str = Prompt.ask(
            f"Enter default number of items to fetch per target (min {MIN_FETCH_ALLOWED})",
            default="50",
        )
        try:
            ...
            if default_count < MIN_FETCH_ALLOWED:
                self.console.print(
                    f"[yellow]Adjusting to minimum fetch count of {MIN_FETCH_ALLOWED}.[/yellow]"
                )
    ...
    def _run_analysis_loop(
        self, platforms: Dict[str, List[str]], fetch_options: Dict[str, Any]
    ):
        """
        ...
        platform_info = " | ".join(
            [f"{p.capitalize()}: {', '.join(u)}" for p, u in platforms.items()]
        )
        self.console.print("\n")
        self.console.print(
            Panel(
        ...
        while True:
            try:
                ...
                self.console.print(
                    "[dim italic]Commands: /exit, /help, /refresh, /loadmore, /add, /remove, /status[/dim italic]"
                )
                ...
                if input_lower == "/exit":
                    ...
                    if self.args.offline:
                        self.console.print(
                            "[yellow]'/refresh' is unavailable in offline mode.[/yellow]"
                        )
                        continue

                    if Confirm.ask(
                        "[yellow]Force refresh data for all targets? (Uses API calls)[/yellow]",
                        default=False,
                    ):
                        query_to_run = Prompt.ask(
                            "Enter query to run with refreshed data", default=last_query
                        ).strip()
                        if query_to_run:
                            ...
                            self.console.print(
                                "[cyan]Refresh cancelled (no query entered).[/cyan]"
                            )
                    ...
                    should_run_analysis, query_to_run, force_refresh = (
                        self._handle_loadmore_command(
                ...
                # Standard Query (No Slash)
                    ...
                    if user_input.startswith("/"):
                        self.console.print(
                            f"[red]Unknown command: {user_input.split()[0]}. Type /help for list.[/red]"
                        )
                        continue
                ...
                self.console.print(
                    f"[bold red]An error occurred:[/bold red] {_escape(str(e))}"
                )
    ...
    def _show_help_table(self):
        ...
        table.add_row(
            "/add platform/user",
            "Add a new target to the session using the session default fetch count.",
        )
        table.add_row(
            "/add platform/user/count",
            "Add a new target fetching a specific number of posts (e.g. /add twitter/elon/100).",
        )
        table.add_row(
            "/remove platform/user",
            "Remove a target from the session (cannot remove the last remaining target).",
        )
        table.add_row(
            "/status", "Show all active targets with post counts and cache freshness."
        )
        table.add_row(
            "/loadmore <n>", "Fetch <n> additional items for current targets."
        )
        table.add_row(
            "/loadmore <p/u> <n>",
            "Fetch more for a specific platform/user (e.g. /loadmore twitter/elon 20).",
        )
        table.add_row(
            "/refresh", "Ignore cache and force a fresh download from all APIs."
        )
    ...
    def _handle_add_command(
        ...
        if len(parts) != 2:
            self.console.print(
                "[red]Invalid format. Use: /add platform/user  or  /add platform/user/count[/red]"
            )
            return
        ...
        if len(segments) == 3:
            ...
            try:
                ...
                self.console.print(
                    f"[red]Invalid count '{count_str}'. Use: /add platform/user/count[/red]"
                )
                return
        elif len(segments) == 2:
        ...
        if platform not in available:
            self.console.print(
                f"[red]Platform '{platform}' is not configured or unavailable. Available: {', '.join(available)}[/red]"
            )
            return
        ...
        if username in platforms.get(platform, []):
            self.console.print(
                f"[yellow]'{platform}/{username}' is already in this session.[/yellow]"
            )
            return
    ...
    def _handle_remove_command(
        ...
        if platform not in platforms or username not in platforms.get(platform, []):
            self.console.print(
                f"[red]'{platform}/{username}' is not in the current session.[/red]"
            )
            return
        ...
        self.console.print(
            f"[green]Removed {platform}/{username} from session.[/green]"
        )
    ...
    def _display_and_save_report(self, result_dict: Dict[str, Any]):
        ...
        content_to_render = (
            Text.from_markup(report_content) if is_error else Markdown(report_content)
        )
        self.console.print(
            Panel(
                content_to_render,
        ...
        if not is_error:
            if not self.args.no_auto_save:
                ...
                save_format = Prompt.ask(
                    "Format?", choices=["markdown", "json"], default=self.args.format
                )
    ...
    def _handle_purge(self):
        ...
        options = {
            "1": ("All", ["cache", "media", "outputs"]),
            "2": ("Cache (Text/Metadata)", ["cache"]),
            "3": ("Media Files", ["media"]),
            "4": ("Output Reports", ["outputs"]),
            "5": ("Cancel", []),
        }
        ...
        if Confirm.ask(
            f"[bold red]This will PERMANENTLY delete all '{name}' data. Are you sure?[/bold red]",
            default=False,
        ):
            for d in dirs:
                ...
                if path.exists():
                    shutil.rmtree(path)
                    self.console.print(
                        f"[green]Successfully purged '{path.name}'.[/green]"
                    )
    ...
    def _handle_cache_status(self):
        ...
        table.add_column(
            "Last Fetched (UTC)", style="green", min_width=19, max_width=19
        )
    ...
    def _handle_loadmore_command(
        ...
        if len(parts) not in [2, 3]:
            self.console.print(
                "[red]Invalid format. Use: `loadmore <count>` or `loadmore <platform/user> <count>`[/red]"
            )
            return False, "", False

        target_str, count_str = (
            (parts[1], parts[2]) if len(parts) == 3 else (None, parts[1])
        )
        try:
        ...
        try:
            ...
            self.console.print(
                f"[red]Invalid target format: '{target_str}'. Use platform/user.[/red]"
            )
            return False, "", False

        if platform not in platforms or username not in platforms.get(platform, []):
            self.console.print(
                f"[red]Target '{target_str}' is not part of the current analysis session.[/red]"
            )
            return False, "", False
        ...
        current_count = target_options.get(
            "count", fetch_options.get("default_count", 50)
        )
        ...
        if last_query:
            self.console.print(
                f"[cyan]Updated {target_str} fetch limit to {new_count} items. Re-running last query...[/cyan]"
            )
            return True, last_query, True  # Force refresh to get new data
        else:
            self.console.print(
                f"[cyan]Updated {target_str} fetch limit to {new_count} items. Enter a query to run an analysis.[/cyan]"
            )
            return False, "", False
    ...
    def _get_cache_info_string(self, platform: str, username: str) -> str:
        ...
        if data.get("timestamp"):
            ...
            age_str = (
                "[green]fresh[/green]"
                if is_fresh
                else f"[yellow]stale ({self._format_cache_age(cached_at.isoformat())})[/yellow]"
            )
